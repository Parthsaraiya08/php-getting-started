...
<?php
    ...
    $visited_services->enqueue("Barcelona Nike Football(LW10).php");
    $visited_services = serialize($visited_services);

    if (isset($_COOKIE['visit_count'])) {
        $visit_count = $_COOKIE['visit_count'];
        $visit_count = unserialize($visit_count);
        if (array_key_exists("Barcelona Nike Football(LW10)", $visit_count)) {
            $visit_count["Barcelona Nike Football(LW10)"] += 1;
        } else {
            $visit_count["Barcelona Nike Football(LW10)s"] = 1;
        }
    } else {
        $visit_count = array();
        $visit_count["Barcelona Nike Football(LW10)"] = 1;
...
<div class="col-span-1 flex flex-col text-center text-white items-center">
        ...
        <p>Nike has also sponsored many other successful track and field athletes over the years,
             such as Carl Lewis, Jackie Joyner-Kersee and Sebastian Coe. The signing of basketball 
             player Michael Jordan in 1984, with his subsequent promotion of Nike over the course of his career,
             with Spike Lee as Mars Blackmon, proved to be one of the biggest boosts to Nike's publicity and sales.</p>
        <br>
    </div>
</div>
