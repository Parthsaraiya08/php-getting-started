...
<?php
    ...
    $visited_services->enqueue("Adidas Football Shoes(MS10).php");
    $visited_services = serialize($visited_services);
    
    if (isset($_COOKIE['visit_count'])) {
        $visit_count = $_COOKIE['visit_count'];
        $visit_count = unserialize($visit_count);
        if (array_key_exists("Adidas Football Shoes(MS10)", $visit_count)) {
            $visit_count["Adidas Football Shoes(MS10)"] += 1;
        } else {
            $visit_count["Adidas Football Shoes(MS10)"] = 1;
        }
    } else {
